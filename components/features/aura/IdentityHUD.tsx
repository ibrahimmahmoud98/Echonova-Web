...
const StatCounter = ({ label, value, suffix }: { label: string; value: number; suffix: string }) => {
    ...
    return (
        <div className="flex flex-col gap-1 md:gap-2">
            <span className="text-[10px] md:text-xs text-[var(--nova-gold)] uppercase tracking-widest">{label}</span>
            <div className="text-2xl md:text-4xl font-mono font-bold text-white flex items-baseline">
                {count}
                <span className="text-xs md:text-sm ml-1 opacity-50">{suffix}</span>
            </div>
...
export const IdentityHUD = () => {
    return (
        <div className="bg-black/80 backdrop-blur-xl border border-white/10 p-4 md:p-8 rounded-2xl w-full max-w-[280px] md:max-w-sm space-y-4 md:space-y-8 shadow-2xl relative overflow-hidden mx-auto">
            {/* HUD Lines */}
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-2 md:pb-4">
                <span className="font-english font-bold text-white text-sm md:text-base">NOVA AURA</span>
                <div className="flex gap-1 items-center">
                    <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-green-500 animate-pulse" />
                    <span className="text-[8px] md:text-[10px] text-green-500 font-mono">النظام نشط</span>
                </div>
            </div>
            ...
            {/* Decorative Code */}
            <div className="font-mono text-[8px] md:text-[10px] text-white/20 select-none text-right" dir="ltr">
