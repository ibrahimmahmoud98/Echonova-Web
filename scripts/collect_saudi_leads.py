...
OUTPUT_FILE = os.path.join(EXPORT_DIR, "بينات الشركات المتوسطة السعودية.xlsx")
...
def verify_email_hunter(email):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log(f"Hunter verification for {email}: status={status}, result={result}, score={score}")
            return status == "valid" or result == "deliverable"
        else:
            log(f"Hunter verifier error {response.status_code}: {response.text}")
    except Exception as e:
        log(f"Hunter verifier exception: {e}")
    return False
...
def scrape_website_direct(domain, website_url):
    ...
    # Try fetching homepage and contact page
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for email in found_emails:
                    ...
                    # Skip common image extensions in case of false regex matches like logo@2x.png
                ...
                # Extract Saudi phone numbers in context of whatsapp
...
def fetch_saudi_companies():
    log("Searching Apollo API for Saudi companies...")
...
def main():
    ...
    candidates = fetch_saudi_companies()
    if not candidates:
    ...
    for idx, comp in enumerate(candidates):
        if len(collected_leads) >= 30:
            log("Reached target of 30 Saudi mid-sized companies!")
            break
        ...
        try:
            ...
            # Check if we got some verified contact data
            # Email (general or marketing), WhatsApp, or Social/LinkedIn marketing managers
            ...
            if has_email or has_whatsapp or has_marketing_li:
                # Let's verify emails using Hunter
                verified_gen_emails = []
                for email in enriched['general_emails'][:2]: # verify top 2
                ...
                verified_mkt_emails = []
                for email in enriched['marketing_emails'][:2]:
                    if verify_email_hunter(email):
    ...
    # Let's design a beautifully formatted Excel sheet
    wb = Workbook()
    ws = wb.active
    ws.title = "الشركات المتوسطة السعودية"
    ...
    bold_data_font = Font(name="Segoe UI", size=10, bold=True, color="111111")
    ...
    ws["A1"] = "إيكونوفا ستوديو - دليل بيانات الشركات السعودية المتوسطة (مستثنى العقارات)"
    ...
    for row_idx, row_data in enumerate(df.values, 3):
        ...
        for col_idx, value in enumerate(row_data, 1):
            ...
            # Highlight verified emails
            if "موثق" in str(value):
                cell.font = Font(name="Segoe UI", size=10, bold=True, color="008000") # Bold Green for verified
    ...
    for col in ws.columns:
        ...
        for cell in col[1:]:
            ...
            # If cell is multiline (like LinkedIn list), size by longest line
            lines = val_str.split('\n')
            for line in lines:
                if len(line) > max_len:
                    max_len = len(line)
